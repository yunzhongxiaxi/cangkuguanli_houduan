...
public class ItemServiceImpl<T> implements ItemService<T>{
    ...
    @Override
    public long getProvideRepositoryID(long goodID,String type){
        List<RepositoryItem> repositoryItems= repositoryItemMapper.selectList(new QueryWrapper<RepositoryItem>().eq("goodId",goodID));
        for(RepositoryItem item:repositoryItems){
            if(repositoryMapper.selectById(item.getRepositoryId()).getClassification().equals(type)){
